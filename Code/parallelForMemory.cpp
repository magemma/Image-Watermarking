 ...
 *
 * Description: this file implements data parallelism (map) using the omp
 * parallel for
 *
 * Example of usage: ./parallelForMemory ../Images/input/
...
high_resolution_clock::time_point start = high_resolution_clock::now();
  ...
  //(4) We edit each image, withour storing the result
#pragma omp parallel for schedule(dynamic) num_threads(parDegree)
  for (int i = 0; i < numPics; i++) { // For each image
    for (int j = 0; j < (int)everything.wm.size();
         j++) { // For each pixel of the watermark
